...
from sklearn.metrics import precision_recall_curve, auc, mean_squared_error, mean_absolute_error, mean_absolute_percentage_error, r2_score

from model.data import MoleDataSet, MoleData, scaffold_split, ratio_split

from model.model import GATGNN, GCNGNN, moleculeformer,moleculeformer_mini,FPonly,FPGNN
...
def set_log(name, save_path):
    ...
    if not log.handlers:
        ...
        log_stream.setFormatter(console_format)
        ...
        log_file = logging.FileHandler(os.path.join(save_path, 'debug.log'))
...
def split_data(data, type, size, seed, log):
    ...
    elif type == 'ratio_random':
        return ratio_split(data, size, seed, log)
    else:
...
def auc_prcauc(label, pred):
    prec, recall, _ = precision_recall_curve(label, pred)
    prcauc = auc(recall, prec)
    auc_score = roc_auc_score(label, pred)
    return [auc_score, prcauc]

def rmse(label, pred):
    result = mean_squared_error(label, pred)
    return [math.sqrt(result)]

def rmse_mse_mae_mape_r2(label, pred):
    # compute MAE
    mae_value = mean_absolute_error(label, pred)
    # compute MAPE
    try:
        mape_value = mean_absolute_percentage_error(label, pred)
    except ZeroDivisionError:
        mape_value = float('nan')
    # compute RMSE
    rmse_value = np.sqrt(mean_squared_error(label, pred))
    # compute R2
    r2_value = r2_score(label, pred)
    # compute MSE
    mse_value = mean_squared_error(label, pred)

    return [rmse_value, mse_value, mae_value, mape_value, r2_value]
...
def get_metric(metric):
    if metric == 'auc':
        ...
        return prc_auc
    elif metric == 'auc_prcauc':
        return auc_prcauc
    elif metric == 'rmse':
        return rmse
    elif metric == 'rmse_mse_mae_mape_r2':
        return rmse_mse_mae_mape_r2
    else:
...
def choose_model(args):
    print("choose model：", args.model_type)
    ...
    elif args.model_type == "FPonly":
        return FPonly(args)

    elif args.model_type == "FPGNN":
        return FPGNN(args)

    elif args.model_type == "moleculeformer":
        return moleculeformer(args)

    elif args.model_type == "moleculeformer_mini":
        return moleculeformer_mini(args)
